...
def run_machine(input_net, first=True):
    ...
    the_image_labels = np.zeros((95, 95))
    ...
    for number_of_clusters in range(2,6):
        img_name = 'clustering_kmeans_' + str(number_of_clusters) + '_' + my_net.getType() + '_autoencoder_' + my_net.getName() + '.png'
        clustering_kmeans(the_image, my_net, np.shape(the_image_labels), img_dir, img_name, number_of_clusters)
